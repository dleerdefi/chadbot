...
// public
exports.getOwnAllBots = catchAsyncErrors(async (req, res, next) => {
	// Parse page and limit from query params, with defaults
	const page = parseInt(req.query.page, 10) || 1; // Default page is 1
	const limit = parseInt(req.query.limit, 10) || 10; // Default limit is 10
	...
	// Fetch users with pagination
	const totalBots = await Bot.countDocuments({ createdBy: req.user.id }); // Get the total number of users
	const bots = await Bot.find({ createdBy: req.user.id })
		.skip(skip)
		.limit(limit)
...
// public
exports.getOwnBotDetails = catchAsyncErrors(async (req, res, next) => {
	if (!mongoose.Types.ObjectId.isValid(req.params.id)) {
		return next(new ErrorHandler("Invalid agent ID", 400));
	}

	const bot = await Bot.findOne({ _id: req.params.id, createdBy: req.user.id });
...
exports.updateOwnBot = catchAsyncErrors(async (req, res, next) => {
	const { username, bio, botRole, botType, botPersonality } = req.body;
	...
	if (botRole && (typeof botRole !== "string" || botRole.trim().length === 0)) {
		return next(new ErrorHandler("Please provide a valid agent role", 400));
	}

	if (botType && (typeof botType !== "string" || botType.trim().length === 0)) {
		return next(new ErrorHandler("Please provide a valid agent type", 400));
	}
	...
	if (
		...
		return next(new ErrorHandler("Please provide a valid agent personality", 400));
	}
...
exports.deleteOwnBot = catchAsyncErrors(async (req, res, next) => {
	if (!mongoose.Types.ObjectId.isValid(req.params.id)) {
		return next(new ErrorHandler("Invalid agent ID", 400));
	}
...
// public
exports.createOwnNewBot = catchAsyncErrors(async (req, res, next) => {
	const { username, botRole, botType, bio, botPersonality } = req.body;

	const totalBots = await Bot.countDocuments({ createdBy: req.user.id }); // Get the total number of users

	if (totalBots >= 10) {
		return next(new ErrorHandler("You can create maximum 10 agents", 400));
	}
	...
	res.status(200).json({ success: true, message: "Agent Created successfully", bot });
});
