...
const ChatWindow = () => {
  ...
  const handleUserStatusUpdate = useCallback(({ userId, status }) => {
    ...
    setAllUsers(prevUsers => prevUsers.map(user => 
      user._id === userId && !user.isBot ? { ...user, isOnline: status === 'online' } : user
    ));
  }, []);
  ...
  const handleBanByUsername = useCallback(async (username) => {
    try {
      const response = await axios.post(`${API_URL}/api/ban-user-by-username`,
        { username },
        {
          headers: {
      ...
      if (response.data.success) {
        showError(`User ${username} has been banned successfully and their messages have been removed.`);
      ...
      showError(`Failed to ban user: ${error.response ? error.response.data.message : error.message}`);
    }
  }, [user, showError]);

  const handleUnbanByUsername = useCallback(async (username) => {
    try {
      const response = await axios.post(`${API_URL}/api/unban-user-by-username`,
        { username },
        {
          headers: {
            'Authorization': `Bearer ${user.token}`,
      ...
      showError(`Failed to unban user: ${error.response ? error.response.data.message : error.message}`);
    }
  }, [user, showError]);

  const handleUnbanUser = useCallback(async (messageUserId) => {
    console.log('Attempting to unban user with ID:', messageUserId);
    try {
      const response = await axios.post(`${API_URL}/api/unban-user`,
      ...
      updateMessages(prevMessages =>
        prevMessages.map(msg =>
          msg.user._id === messageUserId ? { ...msg, user: { ...msg.user, isBanned: false } } : msg
        )
      );
...
const handleSendMessage = useCallback(async (event) => {
      ...
      if (username) {
        switch(command) {
          ...
          case '/unban':
            ...
            break;
          default:
            showError('Invalid command. Available commands: /ban username, /unban username');
        }
      } else {
  ...
  }, [input, inputPrefix, user, socket, sanitizeInput, showError, handleBanByUsername, handleUnbanByUsername, sendMessage]);
  ...
  const handleDeleteMessage = useCallback(async (messageId) => {
    ...
    try {
      ...
      updateMessages(prevMessages => prevMessages.filter(msg => msg.id !== messageId && msg._id !== messageId));
  ...
  const fetchBots = useCallback(async () => {
    ...
    try {
      ...
      console.error('Error fetching bots:', error.response ? error.response.data : error.message);
      showError(`Failed to load bots. Error: ${error.response ? JSON.stringify(error.response.data) : error.message}`);
    } finally {
  ...
  useEffect(() => {
    if (socket) {
      ...
      const handleUserBanned = ({ userId, username }) => {
        ...
        updateMessages(prevMessages => prevMessages.filter(msg => msg.user._id !== userId));
      };
  ...
  const Message = useMemo(() => React.memo(({ message }) => (
    ...
    <div className="chat-window-container">
      ...
      />
        <div className="main-chat">
         ...
         {Object.entries(typingBots).map(([botName, isTyping]) => (
           isTyping && <div key={botName} className="typing-indicator">{botName} is typing...</div>
         ))}
