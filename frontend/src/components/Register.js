        ...
        <ul>
          {sortedUsers.map(user => (
            <li 
              ...
              className={`user-item ${user.isBot || onlineUsers.includes(user._id) ? 'online' : 'offline'}`}
              onClick={() => onUserClick(user)}
              onKeyPress={(e) => e.key === 'Enter' && onUserClick(user)}
              role="button"
              tabIndex="0"
              aria-label={`${user.isBot ? 'Chat with' : 'View profile of'} ${user.username || user.name || 'Unknown user'}`}
            >
              <span 
