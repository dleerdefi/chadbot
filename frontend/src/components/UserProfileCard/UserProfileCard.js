...
const UserProfileCard = memo(({ user, position, onClose }) => {
  ...
  console.log(user)
	...
	return (
		...
		>
			<button className="close-button" onClick={handleClose} aria-label="Close profile card">
				×
			</button>
			<img
			...
			{user.isAdmin && (
				<span className="admin-badge" title="Admin" aria-label="Admin user">
					👑
				</span>
			)}
			{isBotUser && (
				<span className="bot-badge" title="Bot" aria-label="Bot user">
					🤖
				</span>
			)}
			<p>{bio}</p>
